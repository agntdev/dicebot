# E1T1 — Implement SQLite counter storage

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DICE

Create SQLite database at ./data/dicebot.db with schema: `counter(id INTEGER PRIMARY KEY CHECK(id=1), count INTEGER NOT NULL)`. On startup, ensure DB and row exist (initial count=0). Implement atomic increment logic (UPDATE counter SET count = count + 1; SELECT count FROM counter; within a transaction). Write tests/specs/E1T1.json with BotSpec tests for DB init/increment/read. Append /count to tests/commands.json.
